...
def process_prices(df_prices, round, time_limit) -> dict[int, TradingState]:
    ...
    for _, row in df_prices.iterrows():
        ...
        if states.get(time) == None:
            ...
            observations: Observation = Observation(plainValueObservations={}, conversionObservations={})
            listings = {}
            depths = {}
            states[time] = TradingState(None, time, listings, depths, own_trades, market_trades, position, observations)
...
def process_trades(df_trades, states: dict[int, TradingState], time_limit, names=True):
    for _, trade in df_trades.iterrows():
        if str(trade['buyer']) == 'SUBMISSION' or str(trade['seller']) == 'SUBMISSION': continue
...
def simulate_alternative(
    ...
    for symbol, actual_profit in profits_by_symbol[max(profits_by_symbol.keys())].items():
        print(f'Final profit for {symbol} = {actual_profit}')
    total_profit = sum(profits_by_symbol[max(profits_by_symbol.keys())].values())
    print(f"\nSimulation on round {round} day {day} for time {max_time} complete for total profit of {total_profit}")
            ...
            if hasattr(trader, 'logger'):
                if hasattr(trader.logger, 'local_logs') != None:
                    if trader.logger.local_logs.get(time) != None:
                        f.write('{ "sandboxLog": "", "lambdaLog": "' + trader.logger.local_logs[time] + '", "timestamp": ' + str(time) + ' }\n')
                        continue
            if time != 0:
                f.write(f'{time}\n')
...
if __name__ == "__main__":
    trader = Trader()
    trader.local()
    max_time = 199900#int(input("Max timestamp or exact number): ") or 199900)
    if max_time < 10:
        max_time *= 100000
